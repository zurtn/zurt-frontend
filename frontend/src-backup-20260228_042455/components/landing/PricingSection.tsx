        ...
        const mappedPlans: Plan[] = response.plans
          ...
          .map((plan) => {
            ...
            const getCta = (code: string) => {
              ...
              return t('pricing.ctaDefault', { name: plan.name });
            };
            ...
            return {
              name: plan.name,
              code: plan.code,
              subtitle: getSubtitle(plan.code),
              price: formatPrice(plan.priceCents),
              period: plan.priceCents === 0 ? t('pricing.periodForever') : t('pricing.periodMonth'),
              features: plan.features || [],
              cta: getCta(plan.code),
              featured: plan.code === 'pro' || plan.code === 'professional',
            ...
            {plans.map((plan, index) => {
              ...
              return (
                <div
                  key={plan.name}
                  className={cn(
                    "relative flex flex-col p-6 rounded-xl border transition-all duration-300 hover:-translate-y-1",
                    plan.featured
                      ? "bg-card border-primary/40 shadow-lg shadow-primary/10 ring-1 ring-primary/20"
                ...
                >
                    ...
                    <div className="flex -space-x-2">
                      {avatars.map((av, i) => (
                        <div
                          key={i}
                          className={cn(
                            "h-7 w-7 rounded-full border-2 border-card flex items-center justify-center bg-gradient-to-br",
                            av.from, av.to
                          )}
                        >
                          <span className="text-[9px] font-bold text-white leading-none">{av.initials}</span>
                        </div>
                      ))}
                    </div>
                  ...
                  <Button
                    ...
                    className={cn(
                      "w-full mt-auto h-11",
                      plan.featured && "bg-primary hover:bg-primary/90 text-primary-foreground shadow-md shadow-primary/20"
                    )}
                    size="lg"
                  >
                    {plan.cta}
